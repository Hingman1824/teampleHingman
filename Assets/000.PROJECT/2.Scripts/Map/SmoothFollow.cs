...
using UnityEngine;
    ...
    public class SmoothFollow : MonoBehaviour
    {
        ...
        void LateUpdate()
        {
            ...
            listRay.Add(new Ray(pointCenter, targetPosition - pointCenter)); //카메라의 위치헤서 플레이어를 향해 쏘는 레이를 리스트에 저장
            ...
            foreach (Ray ray in listRay) //리스트에 넣어놓은 레이를 반복
            {
                RaycastHit[] hitInfo = Physics.RaycastAll(ray, 100.0f, layerMask); //레이에 맞은 모든 오브젝트 정보를 가져옴
                listHitInfo.Add(hitInfo); //가져온 오브젝트 정보를 레이캐스트힛 리스트에 저장
            }
            /*   지금은 레이를 리스트에 1개만 저장하고 사용해서 바로 레이캐스트힛 리스트에 저장하지만 
             *   레이를 다중으로 사용할 경우  조건문을 통해 다중으로 쏜 레이가 모두 동일한 오브젝트에 맞았는지 확인하고 추가해야 관리하기 편하다.*/
